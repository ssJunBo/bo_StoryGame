...
public class ResourceManager : Singleton<ResourceManager>
{
    ...
    /// </summary>
    public void AsyncLoadResource(string path, OnAsyncObjFinish dealFinish, ELoadResPriority priority,bool isSprite=false, object param1 = null, object param2 = null, object param3 = null, uint crc = 0)
    {
        if (crc == 0)
        {
    ...
    IEnumerator AsyncLoadCor()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < (int)ELoadResPriority.RES_NUM; i++)
            {
                if (m_LoadingAssetList[(int)ELoadResPriority.RES_HIGHT].Count>0)
                {
                    i = (int)ELoadResPriority.RES_HIGHT;
                }
                else if(m_LoadingAssetList[(int)ELoadResPriority.RES_MIDDLE].Count > 0)
                {
