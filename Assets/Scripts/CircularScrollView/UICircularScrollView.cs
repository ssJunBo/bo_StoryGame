...
public class UICircularScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            if (m_MaxCount != -1 && i < lastEndIndex)
            {
                ...
                if (!IsOutRange(rPos))
                {
                    ...
                    Func(m_FuncCallBackFunc, tempCellInfo.obj, i);
                }
                else
                {
    ...
    public virtual void UpdateList()
    {
        for (int i = 0, length = m_CellInfos.Length; i < length; i++)
        {
            ...
            if (cellInfo.obj != null)
            {
                ...
                {
                    Func(m_FuncCallBackFunc, cellInfo.obj, i, true);
                }
            }
        }
    }
    ...
    public void AddItem()
    {
        if (m_Direction == E_Direction.Vertical)
        {
            ...
            m_ContentHeight = m_CellObjectHeight + m_ContentHeight;
            m_ContentWidth = m_ContentRectTrans.sizeDelta.x;
        ...
        {
            m_ContentWidth = m_ContentWidth + m_CellObjectWidth;
            m_ContentHeight = m_ContentRectTrans.sizeDelta.x;
            m_ContentRectTrans.sizeDelta = new Vector2(m_ContentWidth, m_ContentHeight);
            m_ContentRectTrans.anchoredPosition = new Vector2(0, m_ContentRectTrans.anchoredPosition.y);
        }
    ...
    public void UpdateCell(int index)
    {
        ...
        if (cellInfo.obj != null)
        {
            ...
            if (!IsOutRange(rangePos))
            {
                Func(m_FuncCallBackFunc, cellInfo.obj, index);
            }
        }
    }
    ...
    //回调
    protected void Func(Action<GameObject, int> func, GameObject selectObject, int index, bool isUpdate = false)
    {
        int num = index + 1;
        if (func != null)
        {
